#include "FarmingOperationController.h"
...
drogon::Task<>
FarmingOperationController::list(drogon::HttpRequestPtr req,
                                 std::function<void(const drogon::HttpResponsePtr &)> callback) {
  auto [page, pageSize] = LosCommon::LosGetPageParam(req);
  auto db = drogon::app().getDbClient();

  try {
    FarmingOperationMapper itemsMapper(db);
    FarmingOperationMapper countMapper(db);
...
drogon::Task<>
FarmingOperationController::detail(drogon::HttpRequestPtr req,
                                   std::function<void(const drogon::HttpResponsePtr &)> callback,
                                   int32_t id) {
  auto db = drogon::app().getDbClient();
  FarmingOperationMapper mapper(db);
...
drogon::Task<>
FarmingOperationController::create(drogon::HttpRequestPtr req,
                                   std::function<void(const drogon::HttpResponsePtr &)> callback) {
  ...
  std::string err;
  if (!FarmingOperationModel::validateJsonForCreation(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }

  FarmingOperationModel model(bodySnake);
  auto db = drogon::app().getDbClient();
  FarmingOperationMapper mapper(db);
...
drogon::Task<>
FarmingOperationController::update(drogon::HttpRequestPtr req,
                                   std::function<void(const drogon::HttpResponsePtr &)> callback,
                                   int32_t id) {
  auto data = req->getJsonObject();
  if (!data) {
  ...
  std::string err;
  if (!FarmingOperationModel::validateJsonForUpdate(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }
...
drogon::Task<>
FarmingOperationController::del(drogon::HttpRequestPtr req,
                                std::function<void(const drogon::HttpResponsePtr &)> callback,
                                int32_t id) {
  auto db = drogon::app().getDbClient();
  FarmingOperationMapper mapper(db);
...
Json::Value
FarmingOperationController::onFindAllSuc(const std::vector<FarmingOperationModel> &data,
