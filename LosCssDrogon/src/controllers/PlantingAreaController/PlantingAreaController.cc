#include "PlantingAreaController.h"
...
drogon::Task<>
PlantingAreaController::list(drogon::HttpRequestPtr req,
                             std::function<void(const drogon::HttpResponsePtr &)> callback) {
  auto [page, pageSize] = LosCommon::LosGetPageParam(req);
  auto db = drogon::app().getDbClient();

  try {
    PlantingAreaMapper itemsMapper(db);
    PlantingAreaMapper countMapper(db);
...
drogon::Task<>
PlantingAreaController::detail(drogon::HttpRequestPtr req,
                               std::function<void(const drogon::HttpResponsePtr &)> callback,
                               int32_t id) {
  auto db = drogon::app().getDbClient();
  PlantingAreaMapper mapper(db);
...
drogon::Task<>
PlantingAreaController::create(drogon::HttpRequestPtr req,
                               std::function<void(const drogon::HttpResponsePtr &)> callback) {
  ...
  std::string err;
  if (!PlantingAreaModel::validateJsonForCreation(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }

  PlantingAreaModel model(bodySnake);
  auto db = drogon::app().getDbClient();
  PlantingAreaMapper mapper(db);
...
drogon::Task<>
PlantingAreaController::update(drogon::HttpRequestPtr req,
                               std::function<void(const drogon::HttpResponsePtr &)> callback,
                               int32_t id) {
  auto data = req->getJsonObject();
  if (!data) {
  ...
  std::string err;
  if (!PlantingAreaModel::validateJsonForUpdate(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }
...
drogon::Task<>
PlantingAreaController::del(drogon::HttpRequestPtr req,
                            std::function<void(const drogon::HttpResponsePtr &)> callback,
                            int32_t id) {
  auto db = drogon::app().getDbClient();
  PlantingAreaMapper mapper(db);
...
Json::Value
PlantingAreaController::onFindAllSuc(const std::vector<PlantingAreaModel> &data,
