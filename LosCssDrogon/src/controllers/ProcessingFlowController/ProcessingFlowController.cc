#include "ProcessingFlowController.h"
...
drogon::Task<>
ProcessingFlowController::list(drogon::HttpRequestPtr req,
                               std::function<void(const drogon::HttpResponsePtr &)> callback) {
  auto [page, pageSize] = LosCommon::LosGetPageParam(req);
  auto db = drogon::app().getDbClient();

  try {
    ProcessingFlowMapper itemsMapper(db);
    ProcessingFlowMapper countMapper(db);
...
drogon::Task<>
ProcessingFlowController::detail(drogon::HttpRequestPtr req,
                                 std::function<void(const drogon::HttpResponsePtr &)> callback,
                                 int32_t id) {
  auto db = drogon::app().getDbClient();
  ProcessingFlowMapper mapper(db);
...
drogon::Task<>
ProcessingFlowController::create(drogon::HttpRequestPtr req,
                                 std::function<void(const drogon::HttpResponsePtr &)> callback) {
  ...
  std::string err;
  if (!ProcessingFlowModel::validateJsonForCreation(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }

  ProcessingFlowModel model(bodySnake);
  auto db = drogon::app().getDbClient();
  ProcessingFlowMapper mapper(db);
...
drogon::Task<>
ProcessingFlowController::update(drogon::HttpRequestPtr req,
                                 std::function<void(const drogon::HttpResponsePtr &)> callback,
                                 int32_t id) {
  auto data = req->getJsonObject();
  if (!data) {
  ...
  std::string err;
  if (!ProcessingFlowModel::validateJsonForUpdate(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }
...
drogon::Task<>
ProcessingFlowController::del(drogon::HttpRequestPtr req,
                              std::function<void(const drogon::HttpResponsePtr &)> callback,
                              int32_t id) {
  auto db = drogon::app().getDbClient();
  ProcessingFlowMapper mapper(db);
...
Json::Value
ProcessingFlowController::onFindAllSuc(const std::vector<ProcessingFlowModel> &data,
