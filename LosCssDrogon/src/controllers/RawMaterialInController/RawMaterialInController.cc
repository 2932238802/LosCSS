#include "RawMaterialInController.h"
...
drogon::Task<>
RawMaterialInController::list(drogon::HttpRequestPtr req,
                              std::function<void(const drogon::HttpResponsePtr &)> callback) {
  auto [page, pageSize] = LosCommon::LosGetPageParam(req);
  auto db = drogon::app().getDbClient();

  try {
    RawMaterialInMapper itemsMapper(db);
    RawMaterialInMapper countMapper(db);
...
drogon::Task<>
RawMaterialInController::detail(drogon::HttpRequestPtr req,
                                std::function<void(const drogon::HttpResponsePtr &)> callback,
                                int32_t id) {
  auto db = drogon::app().getDbClient();
  RawMaterialInMapper mapper(db);
...
drogon::Task<>
RawMaterialInController::create(drogon::HttpRequestPtr req,
                                std::function<void(const drogon::HttpResponsePtr &)> callback) {
  ...
  std::string err;
  if (!RawMaterialInModel::validateJsonForCreation(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }

  RawMaterialInModel model(bodySnake);
  auto db = drogon::app().getDbClient();
  RawMaterialInMapper mapper(db);
...
drogon::Task<>
RawMaterialInController::update(drogon::HttpRequestPtr req,
                                std::function<void(const drogon::HttpResponsePtr &)> callback,
                                int32_t id) {
  auto data = req->getJsonObject();
  if (!data) {
  ...
  std::string err;
  if (!RawMaterialInModel::validateJsonForUpdate(bodySnake, err)) {
    callback(LosDto::LosResponse::badRequest(err));
    co_return;
  }
...
drogon::Task<>
RawMaterialInController::del(drogon::HttpRequestPtr req,
                             std::function<void(const drogon::HttpResponsePtr &)> callback,
                             int32_t id) {
  auto db = drogon::app().getDbClient();
  RawMaterialInMapper mapper(db);
...
Json::Value RawMaterialInController::onFindAllSuc(const std::vector<RawMaterialInModel> &data,
